...
def sample_tensors(weights_list, sampling_instructions, axes=None, init=None, mean=0.0, stddev=0.005):
    ...
    :param list weights_list: A list of Numpy arrays. Each array represents one of the tensors
            ...
            first tensor.
    :param list sampling_instructions: A list that contains the sampling instructions for each
    ...
    :param list axes: Only relevant if `weights_list` contains more than one tensor.
    ...
    :param list init: Only relevant for up-sampling. Must be `None` or a list of strings
            ...
            'gaussian'.
    :param float mean: Only relevant for up-sampling. The mean of the values that will
            be inserted into the tensors at random in the case of up-sampling.
    :param float stddev: Only relevant for up-sampling. The standard deviation of the
            values that will be inserted into the tensors at random in the case of up-sampling.

    :return list: A list containing the sampled tensors in the same order in which they were given.
    """
    ...
    if (init is not None) and len(init) != len(weights_list):
