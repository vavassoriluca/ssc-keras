...
from object_counting.keras_ssc.data_generator.object_detection_2d_image_boxes_validation_utils import BoxFilter
...
class DataGenerator:
        ...
        with open(self.labels_filename, newline='') as csvfile:
            ...
            for row in csvread:  # For every line (i.e for every bounding box) in the CSV file...
                if self.include_classes == 'all' or int(row[self.input_format.index('class_id')].strip()) in self.include_classes:  # If the class_id is among the classes that are to be included in the dataset...
                    box = list()  # Store the box class and coordinates here
        ...
        for images_dir, image_set_filename, annotations_dir in zip(images_dirs, image_set_filenames, annotations_dirs):
            ...
            for image_id in it:
                ...
                if annotations_dir is not None:
                    self.labels.append(
                        AnnotationParser.parse(os.path.join(annotations_dir, image_id + '.xml'), classes, 'xml'))
    ...
    def parse_json(self,
        ...
        for images_dir, annotations_filename in zip(self.images_dirs, self.annotations_filenames):
            ...
            if verbose:
                it = tqdm(annotations['images'], desc="Processing '{}'".format(os.path.basename(annotations_filename)), file=sys.stdout)
            else:
            ...
            for img in it:
                ...
                if ground_truth_available:
                    ...
                    for annotation in annotations:
                        ...
                        if (not self.include_classes == 'all') and (cat_id not in self.include_classes):
                            continue
