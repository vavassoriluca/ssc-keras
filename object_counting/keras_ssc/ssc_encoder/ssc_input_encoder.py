...
from object_counting.keras_ssc.bounding_box_utils.bounding_box_utils import iou, convert_coordinates
from object_counting.keras_ssc.ssc_encoder.matching_utils import match_bipartite_greedy


class SSCInputEncoder:
    """
    ...
    :param int img_height (int): The height of the input images.
    :param int img_width (int): The width of the input images.
    :param int n_classes (int): The number of positive classes, e.g. 20 for Pascal VOC, 80 for MS COCO.
    :param int predictor_sizes (list): A list of int-tuples of the format `(height, width)`
        containing the output heights and widths of the convolutional predictor layers.
    :param float min_scale (float, optional): The smallest scaling factor for the size of the anchor boxes as a fraction
        ...
        to detect. Must be >0.
    :param float max_scale (float, optional): The largest scaling factor for the size of the anchor boxes as a fraction
    ...
    :param list scales (list, optional): A list of floats >0 containing scaling factors per convolutional predictor layer.
    ...
    :param list aspect_ratios_global (list, optional): The list of aspect ratios for which anchor boxes are to be
    ...
    :param list aspect_ratios_per_layer (list, optional): A list containing one aspect ratio list for each prediction layer.
    ...
    :param bool two_boxes_for_ar1 (bool, optional): Only relevant for aspect ratios lists that contain 1. Will be ignored otherwise.
        If `True`, two anchor boxes will be generated for aspect ratio 1. The first will be generated
        using the scaling factor for the respective layer, the second one will be generated using
        geometric mean of said scaling factor and next bigger scaling factor.
    :param list steps (list, optional): `None` or a list with as many elements as there are predictor layers. The elements can be
    ...
    :param list offsets (list, optional): `None` or a list with as many elements as there are predictor layers. The elements can be
    ...
    :param bool clip_boxes (bool, optional): If `True`, limits the anchor box coordinates to stay within image boundaries.
    :param str matching_type: Can be either 'multi' or 'bipartite'. In 'bipartite' mode, each ground truth box will
    ...
    :param float pos_iou_threshold: The intersection-over-union similarity threshold that must be
        met in order to match a given ground truth box to a given anchor box.
    :param float neg_iou_limit: The maximum allowed intersection-over-union similarity of an
    ...
    :param str border_pixels: How to treat the border pixels of the bounding boxes.
        Can be 'include', 'exclude', or 'half'. If 'include', the border pixels belong
        to the boxes. If 'exclude', the border pixels do not belong to the boxes.
        If 'half', then one of each of the two horizontal and vertical borders belong
        to the boxes, but not the other.
    :param str coords: The box coordinate format to be used internally by the model (i.e. this is not the input format
        of the ground truth labels). Can be either 'centroids' for the format `(cx, cy, w, h)` (box center coordinates, width,
        and height), 'minmax' for the format `(xmin, xmax, ymin, ymax)`, or 'corners' for the format `(xmin, ymin, xmax, ymax)`.
    :param bool normalize_coords: If `True`, the encoder uses relative instead of absolute coordinates.
    ...
    def __call__(self, ground_truth_labels):
        ...
        :return y_encoded: a 3D numpy array of shape `(batch_size, #predictors, #classes)` that serves as the
            ground truth label tensor for training, where `#predictors` is the total number of layers from the SSD
            architecture used as predictors.
        """
        ...
        ##################################################################################
        # Convert matches to predictors counts
        ##################################################################################

        y_encoded_count = []
        pos = 0
        for i, pred_size in enumerate(self.predictor_sizes):
            if isinstance(self.n_boxes, list):
                n_boxes = pred_size[0]*pred_size[1]*self.n_boxes[i]
                y_encoded_count.append(y_encoded[:, pos:pos+n_boxes, :self.n_classes].sum(axis=1))
            else:
                n_boxes = pred_size[0] * pred_size[1] * self.n_boxes
                y_encoded_count.append(y_encoded[:, pos:pos + n_boxes, :self.n_classes].sum(axis=1))
            pos = pos + n_boxes

        if self.diagnostics:
            return np.concatenate(y_encoded_count, axis=1), ground_truth_labels
        return np.concatenate(y_encoded_count, axis=1)
    ...
    def generate_anchor_boxes_for_layer(self,
        ...
        :param list feature_map_size: A list or tuple `[feature_map_height, feature_map_width]` with the spatial
            dimensions of the feature map for which to generate the anchor boxes.
        :param list aspect_ratios: A list of floats, the aspect ratios for which anchor boxes are to be generated.
            All list elements must be unique.
        :param float this_scale: A float in [0, 1], the scaling factor for the size of the generate anchor boxes
            as a fraction of the shorter side of the input image.
        :param float next_scale: A float in [0, 1], the next larger scaling factor. Only relevant if
            `self.two_boxes_for_ar1 == True`.
        :param bool diagnostics: If true, the following additional outputs will be returned:
        ...
        :return np.ndarray: A 4D Numpy tensor of shape `(feature_map_height, feature_map_width, n_boxes_per_cell, 4)` where the
    ...
    def generate_encoding_template(self, batch_size, diagnostics=False):
        ...
        :param bool diagnostics: See the documnentation for `generate_anchor_boxes()`. The diagnostic output
                here is similar, just for all predictor conv layers.

        :return np.ndarray: A Numpy array of shape `(batch_size, #boxes, #classes + 8)`, the template into which to encode
            the ground truth labels for training. The last axis has length `#classes + 8` because the model
            output contains the 4 coordinates for the anchor boxes and the 4 variance values.
        """
        ...
        y_encoding_template = np.concatenate((classes_tensor, boxes_tensor), axis=2)
