...
from object_counting.objects_counter.gt_generation.cython.bboxes_to_count import bboxes2gt_e2e


class GTGenE2E:

    """The :class:`GTGenE2E` class provide a method to convert bounding boxes notation stored in different formats into a count notation as feature maps.
    It is possible to define a grid division to split the count of an object between the cells. (See also Subitizing models).
    """
    ...
    def get_gt(classes, filepath, annotations_ext='xml', input_shape=None, grid_division=None, mode='linear',
        ...
        if input_shape is None or grid_division is None:
            ...
            if mode == 'linear':
                counter = bboxes2gt_e2e(np.array(input_shape), bboxes, len(classes), np.array(grid_division))
            elif mode == 'normal':
                counter = GTGenE2E.bboxes2gt_normal(classes, input_shape, bboxes, grid_division, std_ratio)
            else:
    ...
    def bboxes2gt_normal(classes, input_shape, bboxes, grid_division, std_ratio=(6,6)):
        ...
        grids = GTGenE2E.get_grids(input_shape, grid_division)
        counter = np.zeros((grids.shape[0], grids.shape[1], len(classes)))

        for b in range(len(bboxes)):
            bbox = bboxes[b]
            # Define mu and covariance matrix
            mu = np.array([(bbox[3] - bbox[1]) / 2 + bbox[1], (bbox[4] - bbox[2]) / 2 + bbox[2]])
            S = np.array([[((bbox[3] - bbox[1]) / std_ratio[0]) ** 2, 0], [0, ((bbox[4] - bbox[2]) / std_ratio[1]) ** 2]])
            ...
            for i in range(grids.shape[1]):
                for j in range(grids.shape[0]):
                    low, upp = GTUtils.coordinates_intersection(grids[j, i, :], bbox[1:])
                    if low is None:
                        continue
                    counter[j, i, int(bbox[0])] += mvn.mvnun(low, upp, mu, S)[0] / norm_coeff
    ...
    def get_grids(input_shape, grid_division):
        ...
        :param input_shape: shape of the original image
        :param grid_division: tuple with the number of divisions per dimension
        :return: a list containig the coordinates of the grids
        """
        ...
        for i in range(grid_division[0]):
            ...
            for j in range(grid_division[1]):
                grids_y.append([x_coords[i], y_coords[j], x_coords[i+1], y_coords[j+1]])
            grids_x.append(grids_y)
        return np.array(grids_x)
