...
class CountingModel:
    ...
    """The :class:`CountingModel` object is the core of the Conting Objects with Deep Learning project. It is the parent class of all the models and provides their main methods.

   :param tuple input_shape: the shape of the input
   :param int num_classes: the number output classes
   """

    def __init__(self, input_shape=(224,224,3), num_classes=20):
    ...
    def train(self,
        ...
        if checkpoint_folder is not None:
            ...
            callbacks.append(ModelCheckpoint(filepath=checkpoint_folder+type(self).__name__+'weights_epoch-{epoch:04d}_val_loss-{val_loss:.4f}.h5',
    ...
    def train_generator(self,
        ...
        if checkpoint_folder is not None:
            ...
            callbacks.append(ModelCheckpoint(filepath=checkpoint_folder+type(self).__name__+'_weights_epoch-{epoch:04d}_val_loss-{val_loss:.4f}.h5',
                                             monitor='val_loss', save_best_only=True, save_weights_only=True, mode='auto'))
            callbacks.append(CSVLogger(filename='{}{}_training_log.csv'.format(checkpoint_folder, type(self)),
                                       separator=',', append=True))
