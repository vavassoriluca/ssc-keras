...
class AsoSubFCE2E(CountingModelE2E):
    ...
    def __init__(self, input_shape, base_model='resnet152', num_classes=20, num_hidden=1, hidden_size=500, include_relu=False, freeze_base_net=True, weights=None):
        ...
        model = Conv2D(hidden_size, (1, 1))(self.base_model.layers[-4].output)
        model = BatchNormalization()(model)
        model = ReLU()(model)
        if num_hidden > 1:
            for i in range(num_hidden-1):
                model = Conv2D(hidden_size, (1, 1))(model)
                model = BatchNormalization()(model)
                model = ReLU()(model)
        model = Conv2D(num_classes, (1, 1))(model)
        if include_relu:
            model = ReLU()(model)
